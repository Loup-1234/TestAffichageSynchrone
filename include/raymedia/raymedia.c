...
typedef struct MediaConfig {
    ...
    double maxAllowedDelay[STREAM_COUNT];
    // Maximum allowed delay (in seconds) before an Audio or Video packet is discarded
...
// Calculate total writable space in the buffer, in bytes (not necessarily contiguous)
int GetBufferWritableSegmentSize(const BufferState *state);

// Calculate the largest contiguous writable segment, in bytes
int GetBufferReadableSpace(const BufferState *state);

// Calculate total readable space in the buffer, in bytes (not necessarily contiguous)
int GetBufferReadableSegmentSize(const BufferState *state);

// Calculate the largest contiguous readable segment, in bytes
...
// Write srcData to the buffer. Returns a negative value on error; 
...
// Read data from the buffer into dstData. Returns a negative value on 
...
// Enqueue a packet. Automatically handles reference management.
// Returns true on success, false if the queue is full.

bool DequeuePacket(PacketQueue * queue, AVPacket * dst);
// Dequeue a packet. Automatically handles reference management.
// Returns true on success, false if the queue is empty.

AVPacket *PeekPacket(const PacketQueue *queue);

// Returns a pointer to the first available packet, or NULL if the queue is empty.
...
// Helper function to update the state of the media to the specified new state.

bool HasStream(const MediaContext *ctx, int streamType);

// Checks if the media has an available VIDEO_STREAM or AUDIO_STREAM.
...
MediaProperties GetMediaProperties(MediaStream media) {
    ...
    if (IsMediaValid(media)) {
        ...
        props.durationSec = (double) fmtCtx->duration / AV_TIME_BASE;
        // av_rescale_q(fmtCtx->duration, AV_TIME_BASE_Q, (AVRational) { 1, 1000 });
    ...
    for (int i = 0; i < (int) ctx->formatContext->nb_streams; i++) {
        ...
        if (localCodecParameters->codec_type == AVMEDIA_TYPE_VIDEO &&
            ...
            if (ret == MEDIA_RET_SUCCEED) {
                ...
                ctx->videoOutputImage.data = RL_MALLOC(
                ...
                int allocatedBytes = av_image_alloc(ctx->alignedRgbData, ctx->alignedRgbLinesize, codecCtx->width,
                                                    codecCtx->height, AV_PIX_FMT_RGB24, 32);
            ...
            TraceLog(LOG_DEBUG, "Media Debug: '%s' - Video Codec: resolution %d x %d", fileName,
            ...
            if (!IsAudioDeviceReady()) {
                TraceLog(LOG_WARNING,
                         "MEDIA: '%s' - Audio Codec: raylib audio device is not initialized. Audio will be skipped.",
                         fileName);
                continue;
            }
            ...
            TraceLog(LOG_DEBUG, "Media Debug: '%s' - Audio Codec: %d channels, sample rate %d", fileName,
                     localCodecParameters->ch_layout.nb_channels, localCodecParameters->sample_rate);
        }

        TraceLog(LOG_DEBUG, "Media Debug: '%s' - Audio Codec: %s ID %d bit_rate %lld", fileName, localCodec->name,
                 localCodec->id, localCodecParameters->bit_rate);
    }
...
MediaStream LoadMediaEx(const char *fileName, int flags) {
    MediaContext *ctx = LoadMediaContext(fileName, (MediaStreamReader)
    {
        0
    }
    ,
...
bool UpdateMediaEx(MediaStream *media, double deltaTime) {
    ...
    if (HasStream(ctx, STREAM_AUDIO) && IsAudioStreamProcessed(media->audioStream)) {
        ...
        UpdateAudioStream(media->audioStream, &ctx->audioOutputBuffer.data[ctx->audioOutputBuffer.state.readPos],
                          frameCount);
...
bool AVSeek(MediaStream *media, int64_t targetTimestamp) {
    ...
    const int ret = avformat_seek_file(ctx->formatContext, -1, INT64_MIN, targetTimestamp, INT64_MAX,
...
int AVProcessVideoFrame(const MediaStream *media) {
    ...
    int ret = sws_scale(ctx->swsContext, (const uint8_t * const*) ctx->avFrame->data, ctx->avFrame->linesize, 0,
                        codec->height, ctx->alignedRgbData, ctx->alignedRgbLinesize);
    if (ret <= 0) {
...
bool AVLoadCodecContext(StreamDataContext *streamCtx, const AVCodec *codec, const AVCodecParameters *params) {
    if (streamCtx->codecCtx) {
        TraceLog(LOG_WARNING, "MEDIA: Multiple stream of the same type (%i), only the first will be used.",
                 params->codec_type);
