...
// NOTE : On a retiré le #define RAYMEDIA_STATIC "en dur" ici car CMake le gère maintenant.
...
//--------------------------------------------------------------------------------------------------
// API Configuration (Correction du Linkage)
//--------------------------------------------------------------------------------------------------

// On définit une macro spécifique pour Raymedia, indépendante de RLAPI (Raylib)
#if defined(RAYMEDIA_STATIC)
#define RAYMEDIAAPI             // Vide pour la compilation statique (Ce que tu veux)
#elif defined(_WIN32)
#if defined(BUILD_LIB)
#define RAYMEDIAAPI __declspec(dllexport)
#else
#define RAYMEDIAAPI __declspec(dllimport)
#endif
#else
#define RAYMEDIAAPI             // Vide pour Linux/Mac
#endif
...
typedef struct MediaStreamReader {
    ...
    int64_t (*seekFn)(void *userData, int64_t offset, int whence);

    // Custom seek function pointer (optional, can be NULL)
...
//----------------------------------------------------------------------------------------------
// NOTE: On a remplacé RLAPI par RAYMEDIAAPI dans toutes les fonctions ci-dessous
...
/**
     ...
     */
RAYMEDIAAPI MediaStream LoadMedia(const char *fileName);
...
/**
     ...
     * @return MediaStream on success; empty structure on failure
     */
RAYMEDIAAPI MediaStream LoadMediaEx(const char *fileName, int flags);
...
/**
     ...
     * @return MediaStream on success; empty structure on failure
     */
RAYMEDIAAPI MediaStream LoadMediaFromStream(MediaStreamReader streamReader, int flags);
...
/**
     ...
     */
RAYMEDIAAPI bool IsMediaValid(MediaStream media);
...
/**
     ...
     */
RAYMEDIAAPI MediaProperties GetMediaProperties(MediaStream media);
...
/**
     ...
     */
RAYMEDIAAPI bool UpdateMedia(MediaStream *media);
...
/**
     ...
     * @return true on success; false otherwise
     */
RAYMEDIAAPI bool UpdateMediaEx(MediaStream *media, double deltaTime);
...
/**
     ...
     */
RAYMEDIAAPI int GetMediaState(MediaStream media);
...
/**
     ...
     */
RAYMEDIAAPI int SetMediaState(MediaStream media, int newState);
...
/**
     ...
     */
RAYMEDIAAPI double GetMediaPosition(MediaStream media);
...
/**
     ...
     * @return true on success; false otherwise
     */
RAYMEDIAAPI bool SetMediaPosition(MediaStream media, double timeSec);
...
/**
     ...
     * @return true on success; false otherwise
     */
RAYMEDIAAPI bool SetMediaLooping(MediaStream media, bool loopPlay);
...
/**
     ...
     */
RAYMEDIAAPI int SetMediaFlag(int flag, int value);
...
/**
     ...
     */
RAYMEDIAAPI int GetMediaFlag(int flag);

/**
     * Unload a MediaStream and free its associated memory.
     * @param media Pointer to a valid MediaStream
     */
RAYMEDIAAPI void UnloadMedia(MediaStream *media);
